...
type UpdateContextValue = {
  ...
  /** Download the APK and hand over to the package installer. */
...
const RELEASES_URL = "https://github.com/ej3mpl0/ejflix/releases";
...
export function UpdateProvider({ children }: { children: ReactNode }) {
  ...
  const downloadAndInstall = useCallback(async () => {
    ...
    try {
      ...
      // Android shows the package installer; back in the app the user may retry.
      setPhase("idle");
      setProgress(null);
    } catch (err) {
  ...
  const openRelease = useCallback(() => {
    void api.openExternal(check?.url ?? RELEASES_URL).catch(() => undefined);
  }, [check?.url]);
...
/** Alias kept for the mobile screens (`useUpdates()` in the port notes). */
export const useUpdates = useUpdate;
