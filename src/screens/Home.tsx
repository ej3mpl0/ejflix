...
type View = "home" | "movies" | "series" | "search" | "mylist";
...
export function Home({
  ...
  const catalog = useMemo(() => {
    const pool = [...(data?.all ?? []), ...(data?.series ?? []), ...library, ...results, ...favorites];
  ...
  const patchMovie = (next: Movie) => {
    ...
    setData((prev) => {
      ...
      return {
        ...
        latestSeries: applyRow(prev.latestSeries),
        nextUp: applyRow(prev.nextUp),
        all: applyRow(prev.all),
        series: applyRow(prev.series),
        ...
        {error ? (
          ...
          <div className="px-12 pt-24 pb-16">
            ...
            <FilterBar
              ...
              showSort
            />
            <PosterGrid items={library.length ? library : data?.all ?? []} onOpen={openItem} />
          </div>
        ) : view === "series" ? (
          <div className="px-12 pt-24 pb-16">
          ...
          <>
            ...
            <div className="enter enter-d4 relative z-10 -mt-6 pb-16">
              {favorites.length ? (
                <PosterRow title={t("myList")} items={favorites} onOpen={openItem} onPlay={(item) => void playItem(item)} />
              ) : null}
              {data?.resume.length ? (
                <PosterRow
